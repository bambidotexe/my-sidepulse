...
/// Everything the helper that finishes an update is told. It runs after the app
/// has quit, so it is told all of it up front, as arguments: nothing is
/// interpolated into the script's text.
public struct UpdateInstallPlan: Equatable {
    /// The running app, whose exit the helper waits for.
    public var pid: Int32
    /// The installed bundle, and the new one that takes its place. Same volume,
    /// so each move is one rename.
    public var destination: URL
    public var staged: URL
    /// Where the previous bundle waits until the new version has been seen
    /// running.
    ...
    /// `gui/<uid>/<label>` when the app runs as a launchd job, which is then
    /// how it is started again: an app started by `open` instead would be
    /// nobody's job, and no crash would bring it back.
    public var launchdService: String?
    /// Seconds: for the app to quit, for the new version to show among the
    /// running processes, and how long after that it is looked for once more.
    /// The app itself stops the helper after `K.updateStallNoticeSeconds`,
    /// which is shorter than `quitWait`: the helper's own limit only ever
    /// serves an app too hung to do so.
    ...
    public init(pid: Int32, destination: URL, staged: URL, backup: URL, resultFile: URL, logFile: URL,
                ...
                quitWait: Int = K.updateQuitWaitSeconds, launchWait: Int = K.updateLaunchWaitSeconds,
                settle: Int = K.updateSettleSeconds) {
        self.pid = pid
...
/// What the helper leaves for the next launch to read: one line. The launch
/// that reads it renames the file to `<result>.read`, which is how the helper
/// knows a version that is gone again two seconds later had started properly,
/// and was quit, rather than crashed on its way up.
public enum UpdateResult: Equatable {
    public enum Reason: String {
        /// The new bundle could not be put in place; the previous one never
        /// moved, or was moved back.
        case replace
        /// The new version was put in place and did not start; the previous one
        /// was put back.
        case launch
        /// The previous bundle could not be put back. It is still in
        /// `updates/previous/`.
        case stranded
    }
    ...
    /// `age` is the time since the line was written. Past
    /// `K.updateResultShelfLifeSeconds` it was left behind by an install nobody
    /// is waiting on any more, and opens no window; a negative one means the
    /// clock moved, which proves nothing.
    public static func isNews(age: TimeInterval) -> Bool {
        (0...K.updateResultShelfLifeSeconds).contains(age)
    }
...
/// The helper itself: a POSIX shell script, run by `/bin/sh` in a process group
/// of its own so that it outlives the app that started it. Everything that can
/// be refused (the download, the disk image, the signature, the version, the
/// folder's permissions) was settled while the app was still running; what is
/// left for after the quit is two renames and a launch, and a way back if the
/// new version does not start.
///
/// The three tools are taken from the environment when it names them, which is
/// how the tests stand in for `open`, `ps` and `launchctl`. The app starts the
/// helper with an environment of its own making.

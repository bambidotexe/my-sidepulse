...
/// The Updates group of the Settings window: one version row carrying the last
/// answer as its mark, and one button. The button looks for a release, and once
/// a newer one is known it opens the update window instead. What a check nobody
/// asked for finds shows here exactly as the answer to a press would, and what
/// it fails to find out stays silent.
public struct UpdatePanel: Equatable {
    public enum State: Equatable {
        ...
        /// How the last Install and Relaunch ended, when it did not end with
        /// the new version running.
    ...
    /// nil while a check is in flight: a second press starts no second request.
    /// With a release known every press is the same request, which shows the
    /// update window again.
    ...
    /// The answer to a check nobody asked for. It never interrupts a press; a
    /// repository with nothing published is no news; and after an install that
    /// failed it keeps the reason on the row while the release it finds again
    /// makes the button the retry.

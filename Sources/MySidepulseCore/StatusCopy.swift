...
/// short, names Claude or the command rather than a colour, carries no long
/// dash and ends without a full stop. The words live in `StatusStrings`; what
/// this owns is which sentence a state gets and what tone it carries.
public enum StatusCopy {
    ...
    public static func line(for state: DisplayState) -> Line {
        ...
        case .working:
            return Line(tone: .info, text: t.working)
        case .waiting:
            return Line(tone: .warning, text: t.waiting)
        case .done:
            return Line(tone: .good, text: t.done)
        case .jobRunning:
        ...
        case .split(let alert, _):
            ...
            switch alert {
            case .waiting:
                return Line(tone: .warning, text: t.splitWaiting)
            case .jobFailed:
                return Line(tone: .failure, text: t.splitJobFailed)
            case .done:
                return Line(tone: .good, text: t.splitDone)
            case .jobSucceeded:

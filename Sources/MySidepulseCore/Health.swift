...
/// How a line of the Health table reads. The table answers one question at a glance, whether the app is
/// working, so it has three colours and no fourth: a reading is not a check, and goes in the Information
/// table (`InfoRow`), never here.
public enum HealthLevel: Int, Comparable, Sendable {
    /// Working. Green.
    case good
    /// Not working as it should, and the app still does what it is for: an optional permission denied, an
    /// optional setup not done, a crash this week. The experience is degraded, the core is not. Orange.
    case warning
    /// Not working, and because of it the app cannot do what it is for: a required permission denied, the
    /// one mechanism the feature rests on down. Red, with the stop sign.
    case failure
...
/// One line of the Information table: a reading worth having beside the checks (the last time the thing
/// the app watches happened, a sensor's value), blue, with nothing to judge and nothing to fix.
public struct InfoRow: Equatable, Identifiable, Sendable {
    public let id: String
    public let label: String
    /// A short value: "3 min ago", "112°", "2".

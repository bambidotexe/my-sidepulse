...
/// How one line of the Health page reads. The page answers one question at a glance, whether the app is
...
public enum HealthLevel: Int, Comparable, Sendable {
    ...
    case good
    /// Not as it should be, and the app still does what it is for: an optional permission denied, an
    /// optional setup not done, a check that could not run, a crash this week. The experience is
    /// degraded, the core is not. Orange.
    case warning
    /// Not as it should be, and because of it the app cannot do what it is for: a required permission
    /// denied, the one mechanism the feature rests on down. Red, with the stop sign.
    case failure
...
public struct HealthSummary: Equatable, Sendable {
    /// Lines that are red: each one stops the app from doing what it is for.

...
/// Who signed a bundle. An update is held to one rule: a running app that
/// carries a team identifier is only ever replaced by a copy with a valid
/// signature from that same team. A running app without one (an ad-hoc or
/// unsigned build) has no signer to compare with, so its replacement only has
/// to carry an intact signature.
public enum CodeSignature {
    public enum Failure: Error, Equatable {
        /// The seal is broken, or the bundle is not signed at all. The status
        /// is the Security framework's.
    ...
    /// Checks every architecture and the nested code, strictly: what `codesign
    /// --verify --deep --strict` checks.

...
struct UpdateView: View {
    ...
    var body: some View {
        if let session = controller.session {
            ...
            shell(title: "MySidepulse \(session.release.version.displayString)", status: status(of: session),
    ...
    private func title(of outcome: UpdateResult) -> String {
        switch outcome {
        case .installed(let version): "MySidepulse \(version)"
        case .failed: "MySidepulse \(controller.appVersion)"
        }
    }
    ...
    private func status(of session: UpdateSession) -> String {
        let t = Loc.updateWindow
        switch session.phase {
        case .downloading(let received, let expected):
            guard let expected, expected > 0 else { return t.downloadingUnmeasured }
            return t.downloading(Self.bytes.string(fromByteCount: received), of: Self.bytes.string(fromByteCount: expected))
        case .preparing: return t.preparing
        case .ready: return t.ready
        case .manual: return t.cannotReplaceItself

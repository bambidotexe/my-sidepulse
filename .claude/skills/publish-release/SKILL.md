...
takes a few minutes, most of it Apple's notary service. It publishes; that is the point. Only run it when
...
6. **Installs it in `/Applications`, by the same launch-agent sequence as `scripts/install.sh`**: mount the
...
Check the release page the script printed, and confirm the installed copy sees it:
...
- **After the bump is committed and pushed, before the tag**: the tree already carries the new version.
  ...
  (`git tag -a v<version> -m "MySidepulse <version>"`, push the tag, `gh release create`).

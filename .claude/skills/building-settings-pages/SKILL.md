...
description: Use when adding, removing, renaming, moving or rewording anything in MySidepulse's Settings window (Sources/MySidepulseApp/Settings*.swift) - a setting, a switch, a choice, a status, a group, a page, a hint, a warning, a button, any sentence the window shows - when a new feature needs a user-facing setting, or when building a settings or preferences window for a new macOS app that must look and read like SnappySnap's.
---

Origin: `/Users/Rubens/Projects/snappy-snap/.claude/skills/building-settings-pages`. A rule the owner changes there is changed here too.
...
| You are in | Do this |
|---|---|
| **MySidepulse** | The kit exists. Build from `Sources/MySidepulseApp/SettingsKit.swift`. Never write a `Form`, a `Section`, a `GroupBox` or a hand-made row. Go to *Evolving the window*. |
...
- Group title: a short noun phrase, sentence case (in SnappySnap: "Edges and corners", "Smallest window sizes").
...
## Evolving the window (MySidepulse)

Files, all in `Sources/MySidepulseApp/`: `SettingsKit.swift` (the kit, `SettingsMetrics`),
`SettingsWindow.swift` (`SettingsPageID`: pages, titles, symbols; the toolbar; the height that follows
the page), `Settings…Page.swift` (one per page), `SettingsModel.swift` (the window's view of the
engine: every control reads and writes through it, on the main queue, never the engine directly),
`SettingsSupport.swift` (the screen colour and the sentence of every display state, the QR code),
`StripPreviewView.swift` (the strip replica the pages draw).
...
2. Where it is stored. A value of the app's own: `AppConfig` (`config.json`), where **every key added
   after the first release is optional**, or an older file resets the whole config, ntfy topic
   included. A value another process may set: `UserDefaults`, observed (`showInMenuBar`). What launchd
   owns: `LoginService`. No test pins the roster; a setting the CLI can read gets its field in
   `Platform/Control.swift` and its line in `ControlTests`.
3. The row, from the kit, bound through `SettingsModel`. If it depends on a switch, put it under it
   with `enabled:`.
4. The words, by the ten rules. Ask of each hint and note: would the owner call it useless?
5. A display state's own sentence sits beside its colour in `SettingsSupport.swift`
   (`DisplayState.explanation`); a doctor check's words in `Platform/Doctor.swift`, which the CLI
   prints too.
6. `docs/functional.md` §10 (the table, and its subsections if the setting has words of its own), §12
   for its default, `docs/architecture.md` *Persistence* for a stored key, one line in
   `docs/manual-test-checklist.md`. A request that contradicts a rule written in §10 is a question to
   the owner first.
...
```sh
swift build
rg -n '"[^"]*[—–‒―‐‑−][^"]*"' Sources/MySidepulseApp/Settings*.swift Sources/MySidepulseApp/StripPreviewView.swift Sources/MySidepulseCore/StatusCopy.swift   # nothing; the doctor's details are pinned by DoctorTests
rg -n '[—–‒―‐‑−]' Sources/MySidepulseApp/Settings*.swift                                     # nothing
rg -n '\.font\(\.(caption|callout|footnote|subheadline|title)' Sources/MySidepulseApp/Settings*.swift  # nothing
rg -n 'radioGroup|Form \{|Section|GroupBox|TabView' Sources/MySidepulseApp/Settings*Page.swift  # nothing
rg -n '"[^"]*\b(Option|Command|Control|Shift)\b[^"]*"' Sources/MySidepulseApp/Settings*.swift | rg -v '⌥ Option|⌘ Command|⌃ Control|⇧ Shift'  # nothing
```

Then `make install`, open Settings from the menu bar item (⌘,) and let the owner look. The window has no
automated test.

...
|---|---|
| [functional.md](functional.md) | What the app does: the strip's states and precedence, how each agent's status is read, acknowledgement, phone notifications, terminal jobs, battery, menu, settings, CLI, language, permissions, and every delay and threshold. **Authoritative for behaviour and constants** — update it when behaviour changes. |
...
| [macOS.md](macOS.md) | The operating-system boundary: DiskArbitration and the eject guard, sleep and wake, battery, presence and focus inputs, process inspection, Codex's hooks and their trust, GitHub Copilot's hooks and process, OpenCode's plugin and server, permissions, launchd, the bundle and signing, logging. |
| [pitfalls.md](pitfalls.md) | Traps already fallen into — card slot, LED protocol, Claude Code, Codex, Copilot and OpenCode detection, acknowledgement, ntfy, launchd, hooks — and the open issues. Read before changing any of those areas. |

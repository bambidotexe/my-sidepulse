...
| [ ] | Colours, change a colour, then quit and reopen | The colour is still set, on the strip and on every page's picture; **Reset All Colours** brings back all nine defaults |
| [ ] | Colours, click **Codex working** | The strip rolls blue (`#0a00ff`), the row is selected and Preview plays the same |
| [ ] | Playground, click **Codex working**, then **Claude and Codex working** | Blue roll; then the wave in red for one pass and blue for the next, the rhythm unchanged, on the strip and on the tile alike |
...
| [ ] | System, with Codex installed | A **Codex** group after Claude Code: `Codex hooks` **Enabled** in green, a hint saying what the 12 hooks do, a blue note about trusting them in Codex. **Remove Hooks** turns it orange **Disabled** with a warning; **Set Up Hooks** brings it back. `mysidepulse doctor` agrees both times (`codex hooks`) |

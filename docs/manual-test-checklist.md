...
| [ ] | With the strip plugged in at full brightness and Strip open, run `mysidepulse brightness cycle --steps 3` five times | It prints `LEDs: off` first (the strip goes dark, the menu says off), then `brightness: 33% (LEDs: auto)`, 67 %, 100 %, and off again; the strip and the Strip page's slider follow each step |
| [ ] | Colours, change a colour, then quit and reopen | The colour is still set, on the strip and on every page's picture; **Reset All Colours** brings back all eight defaults |

...
| [ ] | Health | The overview reads **Checking** with a spinner, then **Everything works** in green. Groups Permissions, Claude Code, Strip, Phone, Terminal, App, each row one coloured word, hovering a check shows the doctor's detail. Last hook event, each Claude session and each command are blue and count up. **Check Again** greys out and the overview reads Checking for about half a second. **Copy Report** puts every row on the clipboard with the version, macOS and the topic masked to six characters. Nothing about updates is on the page |
| [ ] | Health, with something wrong | Pull the strip: `SidePulse strip` **Missing** in orange, a warning to plug one in, the overview **1 thing to look at** in orange. Remove the hooks on System: `Claude Code hooks` **Disabled** in red with its stop sign, the overview **Not working: 1 problem** in red. Put both back |
...
| [ ] | Santé in French, hover each row | French labels and words, the doctor's details in French, none with a long dash. **Copier le rapport** copies the page's French rows behind English `[OK]` / `[WARN]` tags |

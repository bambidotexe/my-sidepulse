...
| [ ] | `printf '{"hook_event_name":"UserPromptSubmit","session_id":"bare"}' \| /Applications/MySidepulse.app/Contents/MacOS/mysidepulse hook` | The usage line on stderr, exit 0, and no `bare` line in the journal; with a settings.json still holding the older `… mysidepulse hook` entries (edit one back by hand), `doctor` fails `hooks installed` saying the hooks name no agent, Health lists the Claude Code line as wrong, and Set Up Hooks replaces every such entry with `… hook --agent claude` |

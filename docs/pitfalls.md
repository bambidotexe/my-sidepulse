...
- **Instead.** Every agent's hooks are installed with the flag, Claude Code's as `mysidepulse hook --agent claude`, so the journal line says who fired it whatever process did; `HookConfig.install` replaces every entry carrying the marker, migrated ones included. A hook with no flag is no agent's: it prints its usage on stderr, writes nothing and exits 0, and an entry of that older form reads as ours but wrong (`doctor` and Health say the hooks name no agent) until Set Up Hooks replaces it.
- **Rule.** A hook must say who it is for. Never read the agent from the payload's shape: both agents send the same fields. Never let a bare `hook` exit non-zero or write anything: it still runs inside every turn of an agent whose hooks were not set up again.
...
- **Instead.** Ours is appended after every existing group the first time and replaced in its own place afterwards (`HookConfig.install`): a set-up run again never scrubs ours and re-appends it, which would move a group the user added after ours down one index, untrust it, and then delete its trust table under what had become our key. A table of ours left under an old key (the file rearranged by hand) is recognised by its hash, and removal names our tables after the commands the file actually holds, whichever copy of the app wrote them.
- **Rule.** Never put a Codex entry of ours ahead of a group already there, and never remove ours before appending: a re-install has to leave every other group's index alone.
...
- **Instead.** Only `[hooks.state."<key>"]` tables are read, added and removed, the one shape Codex writes itself; everything else in the file is kept byte for byte, and the file is written only when the trust changes it. A `state` written any other way (an inline table, or a dotted key `"<key>".trusted_hash = …` under `[hooks.state]`, `CodexHookTrust.quotedFirstComponent`) stops the set-up before either file is written, saying to trust the hooks from Codex's `/hooks` screen: a second definition of the same key would make the file invalid for Codex. A file that is not UTF-8 is never written; removing the hooks then still takes them out of `hooks.json`, and says the trust was left.
...
- **Symptom.** A session rolls `working` long after everything is over, until the 2 h staleness, with no finished push (OpenCode has no rescue that would end it sooner).
- **Why.** The main agent's `Stop` came while a helper was live, so the finish was held. The helper then asked a permission; clearing the held finish at that wait, and putting the session back to `working` when the helper acted again, left it working with no finish pending, no hold and no event to come.
- **Instead.** A helper's `PermissionRequest` keeps `pendingDone`; the hold's clocks (`tick`) run only while the session is `working`, so the wait pauses the hold rather than cancelling it, and the helper's next line resumes it. Only a main-agent event cancels a hold (`docs/shared/activity-detection.md` § 14).
- **Rule.** Never clear `pendingDone` on a helper line, and never let `tick` finish a hold while the session waits: a prompt still open is not a finish.
...
- **Instead.** The hook says which agent speaks, and `Trim.journalEvent(fromHookPayload:agent:loggedAt:)` passes only that agent's own events (`HookConfig.events(for:)`: Claude Code's 15, Codex's 12), in the spelling both agents send (`SessionStart`, which a real Codex 0.157 turn sent too); the snake case of Codex's configuration is no event name. Any other name is a `ParseError` line that keeps the payload's first bytes and none of its fields. Copilot's event is the hook's `--event` argument alone, in Copilot's own spelling, never the payload's `hook_event_name`; OpenCode's trim maps only OpenCode's own events onto the journal's names.
...
### A wrapper's exit can overtake its own end line
- **Symptom.** Without the ordering below, `mysidepulse run -- false` can end with nothing on the strip instead of amber: a race by construction, not one seen on the strip.
- **Why.** The wrapper appends its `JobEnd` line and exits at once. The line reaches the app through the journal's kqueue and the tailer's queue; the exit through the process watcher's kqueue. Either can reach the main queue first, and an exit applied first clears the job while it still reads as running, so the end line finds no job.
- **Instead.** `Engine.handleProcessExit` asks the tailer to catch up first (`JournalTailer.catchUp`): it reads what the file holds on its own queue, delivers it, and only then hands the exit back to the main queue, behind the lines. The line was on disk before the process exited, so it is always read first.
- **Rule.** Never apply a process exit ahead of the journal: whatever a process wrote before it died is older than its death.
...
- **Instead.** Two checks of the same rule, `ProcWalk.hostingAgent(in:)` over the shell's chain. The CLI writes no `JobBegin` for such a shell (`JobJournal.begin`, for `job begin` and `run`), deciding while the shell is certainly alive, so the journal never carries the line. The app drops a `JobBegin` line whose watched pid it places under an agent when it reads it, live or replayed (`JobStore.apply`, fed by `Engine.ingest`), logging `job ignored: shell <pid> runs under <agent>` once per shell, which catches the lines an older CLI wrote. The agent's own session is what shows its work.
...
- **Symptom.** A `job end` that never arrived (the CLI missing during an install or an update, a journal line that could not be written, a snippet re-read mid-command) left the strip violet until the next command in that shell; a timer on the job, the other way round, took the colour off a three-hour build while it ran.
...
- **A `mysidepulse run` wrapper killed while its child survives leaves the command running unlit.**

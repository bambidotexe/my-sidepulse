...
- **Symptom.** A Copilot turn stopped with Ctrl+C, or whose model call gave up, rolling on until the Copilot process exits or the 2 h backstop.
...
- **A Copilot turn ended by Ctrl+C, Esc Esc or a failed model call is read from its `events.jsonl` within about 35 s** (the 20 s quiet gate and the 15 s recheck), not at once: no hook reports it. Until then an interrupted turn rolls, and a failed one rolls before its amber.
- **A Copilot dialog cancelled with Ctrl+C keeps its amber**, as a Claude dialog cancelled with Esc does: only a working turn is checked against the file, and approving a permission fires no hook, so a session reads `waiting(permission)` until the tool's `postToolUse`, which for a long `bash` comes when Copilot hands it back as still running, about 30 s later.

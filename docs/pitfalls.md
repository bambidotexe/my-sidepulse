...
- **A Copilot turn ended by Ctrl+C, Esc Esc or a failed model call, and a Copilot prompt answered, are read from its `events.jsonl` within about 35 s** (the 20 s quiet gate and the 15 s recheck), not at once: no hook reports them. Until then an interrupted turn rolls, a cancelled or answered prompt stays amber, and a failed turn rolls before its amber.

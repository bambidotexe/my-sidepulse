...
- **Why.** A TUI session's hooks are spawned by Codex's managed daemon (`codex app-server --listen unix:// --managed-daemon`, under `~/.codex/packages/app-server-daemon/`, parented by launchd), not by the TUI, and the hook records the nearest `codex` process in its ancestry: the daemon. One daemon serves every TUI of the user and outlives them all; it keeps a thread loaded for 30 min after its last subscriber, so a killed TUI does not even stop a running turn. `codex exec` and the ChatGPT app run their own `codex`, which is the session's own process. So a lost `Stop`, a lost `Interrupt`, an errored turn or a hook Codex stopped trusting cost the full 2 h backstop: nothing keyed on the pid can end a TUI session.
- **Instead.** The session's rollout is its transcript: every Codex hook names it (`transcript_path`), and its turn markers (`task_started`, `task_complete`, `turn_aborted`) balanced exactly across the 399 turns of that day. A quiet working Codex session is checked against the rollout's last 64 KB every 15 s and at launch (`SessionStore.codexCandidates`, `CodexRolloutTail`, `Engine.checkCodexRollouts`): an end marker stamped after the last main-agent event, or naming its turn, ends the turn; a `task_started` with no end keeps it alive, counted from the rollout's last line. `ProcWalk.isCodexDaemon` tells the daemon apart by its arguments (`app-server`) or its install folder, and the startup prune keeps a session on it for the launch check to decide. The kqueue on the daemon's pid stays: its death does end every session it hosted.

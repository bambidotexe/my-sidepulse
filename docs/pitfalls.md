...
- **Symptom.** A Copilot turn stopped with Ctrl+C, or whose model call gave up, rolling on until the Copilot process exits or the 2 h backstop; a permission prompt cancelled with Ctrl+C left amber, saying "needs you" for a turn the user cancelled.
- **Why.** Approving a permission or answering `ask_user` fires no hook: the next `postToolUse` is the answer. Ctrl+C and Esc Esc fire nothing, and a failed model call fires only `errorOccurred`, whose `recoverable` flag is true on retries that then succeed too (three connect timeouts in session `fe073462` each fired one before the turn went on), so no hook tells a failed turn apart. `agentStop` fires on a natural end only.
- **Instead.** The session's `events.jsonl` records each: `abort` for the interrupt, `session.error` for the failure (the retried errors write only their hook's mirror, and only the last failure writes a `session.error`), `session.shutdown` for the session closing, and a `hook.start` mirror of the session's own `agentStop` for the natural end. A quiet working Copilot session is checked against the file's last 64 KB every 15 s and at launch, like a Codex rollout (`SessionStore.copilotCandidates`, `CopilotTranscriptTail`, `Engine.checkCopilotTurns`): an end marker stamped after the last main-agent event ends the turn, dark for an abort or a close, green with its push for the `agentStop`, `waiting(error)` with its push for the `session.error` (`SessionStore.failTurn`, journaled as `turn-failed`); a step of the turn at work keeps the session alive from the file's last line. A session waiting on a permission or a question is checked too, from when the wait began (`SessionStore.copilotWaitCandidates`, `CopilotTranscriptTail.waitDecision`): an `abort` stamped after the wait began is the prompt cancelled (`SessionStore.abandonWait`, dark, journaled as `turn-abandoned`); anything else leaves the wait to the next hook. The only interrupt seen on this Mac was exactly that, Ctrl+C at an open permission prompt (session `fe073462`).
- **Rule.** Never end a Copilot turn on `errorOccurred`, on `assistant.turn_end` (it ends every model call) or on the file's modification time. Read the lines' types, stamps, and a `hook.start`'s hook name and session only: the rest of each line is the conversation, and it is never logged.
...
- **A Copilot turn ended by Ctrl+C, Esc Esc or a failed model call is read from its `events.jsonl` within about 35 s** (the 20 s quiet gate and the 15 s recheck), not at once: no hook reports it. Until then an interrupted turn rolls, a cancelled prompt stays amber, and a failed turn rolls before its amber.
- **Approving a Copilot permission fires no hook**, so the session reads `waiting(permission)` until the tool's `postToolUse`, which for a long `bash` comes when Copilot hands it back as still running, about 30 s later. The file's `permission.completed` is not read as the answer: only an `abort` ends a wait.
